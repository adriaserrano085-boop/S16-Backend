...
def send_activation_email(to_email: str, token: str):
    """
    Sends an account activation email using Brevo (Sendinblue) API.
    """
    activation_link = f"{FRONTEND_URL}/activate?token={token}"
    
    subject = f"Activa tu cuenta de {APP_NAME} 🚀"
    ...
    <body>
        <div class="container">
            ...
            <div class="content">
                <div class="greeting">¡Bienvenido a {APP_NAME}!</div>
                <div class="message">
                    Has creado una cuenta nueva. Para poder acceder, necesitamos que confirmes tu dirección de correo electrónico.
                </div>
                <div class="button-wrapper">
                ...
                <div class="message">
                    ...
                    Una vez activada, un administrador revisará tu cuenta para asignarte los accesos correspondientes a tu rol (jugador, familiar, staff, etc).
                </div>
                <div class="security-note">
                    <b>Nota de seguridad:</b><br>
                    Este enlace caducará en 48 horas. Si no has sido tú quien ha creado esta cuenta, puedes ignorar este correo.
                </div>
            </div>
            <div class="footer">
                <p>¡Nos vemos pronto en la app!</p>
    ...
    if not BREVO_API_KEY:
        print(f"WARNING: BREVO_API_KEY missing. Cannot send activation email to {to_email}.")
    ...
    send_smtp_email = sib_api_v3_sdk.SendSmtpEmail(
        to=[{"email": to_email}],
        reply_to={"email": SENDER_EMAIL, "name": APP_NAME},
        headers={"Some-Custom-Name": "activation-email"},
        html_content=html_content,
        sender={"email": SENDER_EMAIL, "name": APP_NAME},
        subject=subject
    )
    ...
    try:
        api_response = api_instance.send_transac_email(send_smtp_email)
        print(f"INFO: Activation email sent via Brevo. Response: {api_response}")
        return True
    except ApiException as e:
        print(f"ERROR: Exception when calling TransactionalEmailsApi->send_transac_email for activation: {e}")
        print(f"DEBUG: Activation Link for {to_email}: {activation_link}")
        return False
    except Exception as e:
        print(f"ERROR: General error sending activation email via Brevo to {to_email}: {e}")
        print(f"DEBUG: Activation Link for {to_email}: {activation_link}")
        return False
